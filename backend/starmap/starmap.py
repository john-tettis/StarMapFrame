...
if __name__ == '__main__':
    ...
    # Watermark
    if MODE != "PROD":
        image.add(image.image(href="http://localhost:5000/assets/watermark.png", size=("100%", "100%"), opacity=".05"))
    ...
    if is_heart == "True":
        image.add(image.image(href="http://localhost:5000/assets/heart.png", size=("100%", "100%"), insert=(0, -80)))
    
    if qrCode.strip():
        if MODE == "PROD":
            image.add(image.image(href=qrCode, size=("64px", "64px"), insert=("170mm", str(height-25)+'mm')))
        else:
            image.add(image.image(href="http://localhost:5000/assets/qr.jpeg", size=("64px", "64px"), insert=("170mm", str(height-25)+'mm')))
